  ...
  return (
    <Dialog onOpenChange={handleOpenChange}>
      <DialogTrigger asChild>
        <button className="px-6 py-3 text-sm font-bold text-slate-700 bg-white border border-slate-200 rounded-full hover:bg-slate-50 hover:text-emerald-700 transition-all shadow-sm hover:shadow-md active:scale-95">
          Ver Demo
        </button>
      </DialogTrigger>

      {/* RESPONSIVE CONTAINER:
         - Mobile: w-full h-[100dvh] (Full screen, no rounded corners)
         - Desktop (sm+): max-w-6xl h-auto max-h-[85vh] rounded-2xl
      */}
      <DialogContent
              ...
              {step === "simulation" && (
                <motion.div
                  ...
                  <div className="grid grid-cols-1 md:grid-cols-12 gap-6 h-full">
                    {/* The "Action" Card - Full Width on Mobile */}
                    <button
                      onClick={() => {
                        setIsOptimized(true);
                    ...
                    {/* Chart - Hidden on small mobile screens if space is tight, visible on larger phones/tablets */}
